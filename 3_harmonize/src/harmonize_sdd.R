harmonize_sdd <- function(raw_sdd, p_codes){
  ...
  # Note that ActivityCommentText not included because fail comments were
  # mostly about non-SDD measurements
  ...
  sdd_specialcharacters_removed <- sdd_fails_removed %>%
    ...
    mutate(
      ResultMeasureValue = if_else(
        condition = index %in% sdd_specialcharacters$index,
        true = abs(as.numeric(
          str_extract(ResultMeasureValue_original, "-?\\d+(\\.\\d+)?")
        )),
        false = ResultMeasureValue
      )
    )
  ...
  mdl_updates <- sdd_specialcharacters_removed %>%
    # only want NAs in the altered value column and where there is a `<` and
    # a number in the original values column and no alphabetical characters...
    ...
    # just in case.
    mutate(
      harmonized_value = abs(as.numeric(
        str_extract(ResultMeasureValue_original, "-?\\d+(\\.\\d+)?")
      )),
      harmonized_units = ResultMeasure.MeasureUnitCode) %>%
    # Keep important data
    select(index, harmonized_value, harmonized_units)
  
  # What % of samples had special characters?
  print(
    paste(
  ...
  # the MDL ("less than") values, we can coerce the ResultMeasureValue that
  ...
  # this will cause coercion errors, which is expected, as some values have
  # additional characters in them that cause this step to have errors.
  ...
  # Replace "harmonized_value" field with these new values
  sdd_greaterthan_added <- sdd_mdls_added %>% 
  ...
  gap_fill_comment_cols <- c("ActivityCommentText","ResultCommentText",
  ...
  negate_bottom_text <- paste(
    "no bottom", "not.*bottom", "couldn't.*bottom", "could not.*bottom",
    "unable to.*bottom", "too deep", "too shallow", "doesn't reach",
    "did not reach", "blocked", "hidden", "disappeared", "hose frozen",
    "pump frozen", "cable not long enough", "cord not long enough",
    "won't stay on bottom", "current too strong", "too windy", "fast current",
    "flooding", "readings not on bottom", "measurements not collected",
    "sample taken at", "depth too great", "can't get true bottom",
    "seen on bottom: no", "bottom reading not taken", "afraid to touch bottom",
    "forgot bottom reading", "didn't hit bottom", "did not hit bottom",
    "vegetation blocks", "turbid", "probe not on bottom", "sonde.*not.*bottom",
    "hydrolab not on bottom", "profile not taken to bottom",
    "unable to collect bottom", "bottom.*not.*collected", "bottom.*not.*recorded",
    sep = "|")
  ...
  sdd_approx_added <- sdd_greaterthan_added %>%
    ...
    mutate(
      ...
      # Create a bottom tag to detect bottom language detected in relevant
      # comment columns to be used for flagging
      ...
      approx_flag = case_when(
        ...
        # If the harmonized value is NA and there is bottom language and no
        # negate language, and ActivityBottomDepthHeightMeasure.MeasureValue
        # is not NA or 0 set flag to 2
        ...
        # If the harmonized value is NA and there is no bottom language or there
        # is negate language, and ActivityDepthHeightMeasure.MeasureValue is not
        # NA or 0 set flag to 1
        is.na(harmonized_value) &
        ...
        # it is possible to get NA values here, but we will drop them in the next step
        .default = NA_integer_ 
      ),
  ...
  # % of records backfilled from depth cols?
  print(
    paste(
      round((nrow(filter(sdd_approx_added, approx_flag != 0))) /
              nrow(sdd_greaterthan_added) * 100, 3),
      "% of samples have missing values or units that can be filled from depth measure columns."
    )
  )
  ...
  # % of samples with remaining NAs removed?
  print(
    paste(
  ...
  sdd_bottom_depth <- converted_units_sdd %>%
    # Filter for samples that have bottom text in the comment columns and
    # have no information in the bottom depth columns.
    filter(
      bottom_tag == 1 & negate_bottom_tag != 1 &
        (is.na(ActivityBottomDepthHeightMeasure.MeasureValue) &
  ...
  print(
    paste0(
      ...
      "% of samples with bottom contact where bottom depth measurements can be back filled."
    )
  )
  
  # Harmonize and gap fill ActivityBottomDepthHeightMeasure.MeasureValue field
  ...
  sdd_bottom_depth_added <- converted_units_sdd %>%
    mutate(
      ...
      # Flag 1 = bottom depth value filled in with harmonized_value due to
      # indication that SD hit bottom
      ...
      harmonized_bottom_depth_value = if_else(
        depth_flag == 1,
        harmonized_value,
        as.numeric(ActivityBottomDepthHeightMeasure.MeasureValue)
      ),
      harmonized_bottom_depth_unit = if_else(
        depth_flag == 1,
        harmonized_units,
        as.character(ActivityBottomDepthHeightMeasure.MeasureUnitCode)
      ),
  ...
  # Text to use to identify records unrelated to SDD
  ...
  print(
    paste0(
      round((nrow(sdd_bottom_depth_added) - nrow(sdd_relevant)) /
              nrow(sdd_bottom_depth_added) * 100, 3),
  ...
  # Time tagging: Categorize timestamps to identify optimal conditions for
  # Secchi disk depth measurements.
  # Tag 0 (10am-2pm) represents the ideal time range for measurements due to
  # consistent lighting conditions. This helps in assessing data quality and
  # reliability of Secchi disk depth readings.
  sdd_time_tag <- sdd_relevant %>%
    mutate(
      harmonized_local_time_temp = as.POSIXct(harmonized_local_time,
                                              format = "%Y-%m-%d %H:%M:%S",
                                              # Prevent time zone conversions
                                              tz = "UTC"), 
      time_tag = case_when(
        # tag 0: time is within 10a-2p and is not 11:59:59
        (format(harmonized_local_time_temp, "%H:%M:%S") != "11:59:59" &
           hour(harmonized_local_time_temp) >= 10 &
           hour(harmonized_local_time_temp) < 14) ~ 0,
        # tag 1: time is not within 10a-2p and is not 11:59:59
        (format(harmonized_local_time_temp, "%H:%M:%S") != "11:59:59" &
           (hour(harmonized_local_time_temp) < 10 |
              hour(harmonized_local_time_temp) >= 14)) ~ 1,
        # tag 2: time is 11:59:59 or NA
        (format(harmonized_local_time_temp, "%H:%M:%S") == "11:59:59" |
           is.na(harmonized_local_time_temp)) ~ 2,
        .default = NA_integer_
      )
    ) %>% 
    # Remove temporary column
    select(-harmonized_local_time_temp)
  
  # Scope tagging: Identify whether a viewscope was used during Secchi disk
  # depth measurements.
  # Using a viewscope can significantly improve measurement accuracy by
  # reducing surface glare and wave effects. This tagging helps in assessing
  # the reliability and consistency of the  Secchi disk depth data across
  ...
  sdd_scope_tag <- sdd_time_tag %>%
    ...
    #      1 = P-code does not match viewscope, No indication that a viewscope
    #          was used, or there is negate language regarding viewscope used
    mutate(scope_tag = case_when(
      # Negate language detected
      rowSums(
        across(all_of(scope_comment_cols),
               ~grepl(negate_scope_text, .x, ignore.case = TRUE))) > 0  ~ 1,
      # Scope language detected or correct USGS code
      rowSums(
        across(all_of(scope_comment_cols),
               ~grepl("scope", .x, ignore.case = TRUE))) > 0 | USGSPCode == 72187 ~ 0,
      # If negate language is not detected, scope language is not detected and
      # the p-code does not match the one for view scope default to 1
      .default = 1)
    )
  ...
  rm(sdd_relevant, sdd_time_tag, sdd_scope_tag)
  gc()
  ...
  # Comment columns to check when flagging
  ...
  flagged_sdd <- tiered_methods_sdd %>%
    mutate(
      # mdl_flag: 
      # 0 = value greater than 0.01m with NO "<" character value
      ...
      environmental_indicator_tag = if_else(
        rowSums(
          across(all_of(flag_comment_cols),
                 ~grepl(environmental_indicator, .x, ignore.case = TRUE))) > 0,
        1,
        0),
      # field_flag
      # 0 = environmental indicator present
      # 1 = no environmental indicator present
      field_flag = case_when(
        # Environmental condition text present
        environmental_indicator_tag == 1 ~ 0,
        # No environmental condition text present
        .default = 1  
      ),
  ...
  print(
    paste0(
      round((nrow(tiered_methods_sdd) - nrow(cleaned_flagged_sdd)) /
              nrow(tiered_methods_sdd) * 100, 3),
  ...
  # 5. This conservative approach aims to minimize the inclusion of potentially
  ...
  print(
    paste0(
      round((nrow(cleaned_flagged_sdd) - nrow(realistic_sdd)) /
              nrow(cleaned_flagged_sdd) * 100, 3),
  ...
  print(
    paste0(
      round((nrow(filter(realistic_sdd, mdl_flag %in% c(2, 3)))) /
              nrow(realistic_sdd) * 100, 3),
  ...
  # We take the mean and CV of those values here
