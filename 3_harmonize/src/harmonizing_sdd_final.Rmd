...
walk(list.files("3_harmonize/src/", pattern = "\\.R$", full.names = TRUE), source)
```
...
#                path = "../AquaMatch_harmonize_WQP_old/scratch/data/sdd_data.feather",
#                overwrite = TRUE)

# update before PR ***
p2_wqp_data_aoi_sdd <- read_feather("../AquaMatch_harmonize_WQP_old/scratch/data/sdd_data.feather") # need to update this ***
```
...
#                overwrite = TRUE)
p1_char_names_crosswalk <- readRDS("../AquaMatch_harmonize_WQP_old/scratch/data/p1_char_names_crosswalk_data.rds")
...
#                path = "../AquaMatch_harmonize_WQP_old/scratch/data/p2_site_counts_data.rds",
#                overwrite = TRUE)
p2_site_counts <- readRDS("../AquaMatch_harmonize_WQP_old/scratch/data/p2_site_counts_data.rds")
...
#                path = "../AquaMatch_harmonize_WQP_old/scratch/data/p1_wqp_inventory_aoi_data.rds",
#                overwrite = TRUE)
p1_wqp_inventory_aoi <- readRDS("../AquaMatch_harmonize_WQP_old/scratch/data/p1_wqp_inventory_aoi_data.rds")
...
walk2(.x = fail_counts,
      .y = names(fail_counts),
      .f = ~ ggsave(filename = paste0("../AquaMatch_harmonize_WQP_old/scratch/figures/sdd_",
                                      .y,
                                      "_fail_pie.png"),
...
# have to fill in NAs here, then MDL, approx, and greater than will behave as expected

  # make sure to keep "<" data (MDL greater than step)
  # make sure to keep ">" data (greater than step)
  # make sure to keep "=" data

# Matchup table for expected sdd units in the dataset
unit_conversion_table <- tibble(
  ResultMeasure.MeasureUnitCode = c("mm", "cm", "in", "ft", "m", "meter"),
  conversion = c(.001, .01, .0254, .3048, 1, 1))

unit_table_out_path <- "../AquaMatch_harmonize_WQP_old/scratch/data/sdd_unit_table.csv"
...
sdd_fails_removed %>%
  ...
  plot_unit_pie() %>%
  ggsave(filename = "../AquaMatch_harmonize_WQP_old/scratch/figures/sdd_unit_drop_pie.png",
         plot = .,
         width = 6, height = 6, units = "in", device = "png")
...
# during the unit harmonization step I need to be more careful
  # do not nuke things with character values in it
...
View(
  ...
  select(ResultMeasureValue_original, ResultMeasure.MeasureUnitCode, ResultMeasureValue,
         harmonized_value, ResultCommentText,
    ...
    group_by(ResultMeasureValue_original) %>% 
    mutate(count = n())
)

# filtered_data <- sdd_mdls_added %>%
#   filter(!index %in% mdl_updates$index,
#          (is.na(ResultMeasureValue) & 
#           grepl("[0-9]", ResultMeasureValue_original) &
#           grepl(approx_text, ResultCommentText, ignore.case = TRUE)))
...
#           grepl(approx_text, ResultCommentText, ignore.case = TRUE))) %>% 
#   select(ResultMeasureValue_original, ResultMeasureValue, ResultCommentText)

# View(filtered_data)
```

```{r}
comment_cols <- c("ResultMeasureValue_original","ResultCommentText","ActivityCommentText")

unique_values_list <- map(comment_cols, ~{
  sdd_harmonized_values %>%
    # filter(grepl("bottom", !!sym(.x), ignore.case = TRUE)) %>%
    group_by(!!sym(.x)) %>% 
    summarise(count = n()) %>% 
    rename(comment = !!sym(.x))
}) %>% 
  bind_rows() %>% 
  mutate(comment = trimws(tolower(comment))) %>% 
  group_by(comment) %>%
  summarise(count = sum(count)) %>%
  arrange(desc(count)) 

# Create a function to format each row
format_row <- function(comment, count) {
  paste0("Comment: ", comment, "\nCount: ", count, "\n", "-", "\n")
}

# Apply the formatting function to each row and combine into a single string
formatted_text <- mapply(format_row, unique_values_list$comment, unique_values_list$count) %>%
  paste(collapse = "\n")

# Write the results to a text file
writeLines(formatted_text, "../AquaMatch_harmonize_WQP_old/scratch/data/unique_values_results.txt") 
```
...
test_df <- sdd_mdls_added %>% 
    select(index, ResultMeasureValue_original, ResultMeasureValue, ResultMeasure.MeasureUnitCode,ResultCommentText)
  test_word <- "value approx"
  unique(test_df %>% filter(grepl(test_word, ResultMeasureValue_original, ignore.case = T)) %>% 
           pull(ResultMeasureValue_original))
  unique(test_df %>% filter(grepl(test_word, ResultCommentText, ignore.case = T)) %>% 
           pull(ResultCommentText))
...
print(
  paste(
    round((nrow(greater_vals)) / nrow(sdd_approx_added) * 100, 1),
...
test_df <- sdd_harmonized_values %>% 
    select(
      index, ResultMeasureValue_original, ResultMeasureValue, 
      ResultMeasure.MeasureUnitCode, ResultCommentText,
      ActivityCommentText,
    ...
    filter(
      (is.na(harmonized_units)&is.na(harmonized_value)) &
      (grepl(bottom_text, ResultMeasureValue_original, ignore.case=T)|grepl(bottom_text, ResultCommentText, ignore.case=T)) &
      (!is.na(ActivityBottomDepthHeightMeasure.MeasureValue)|!is.na(ActivityDepthHeightMeasure.MeasureValue)) 
    )
  
  test_word <- "approx"
  unique(test_df %>% filter(grepl(test_word, ActivityCommentText, ignore.case = T)) %>% 
           pull(ActivityCommentText))
  unique(test_df %>% filter(grepl(test_word, ResultCommentText, ignore.case = T)) %>% 
           pull(ResultCommentText))
...
depth_check_out_path <- "../AquaMatch_harmonize_WQP_old/scratch/data/sdd_depth_check_table.csv"
...
depth_counts_out_path <- "../AquaMatch_harmonize_WQP_old/scratch/data/sdd_depth_counts.csv"
...
# scope_tag
scope_text <- paste0(c("scope", "viewscope", "viewscope: yes", "-Viewscope used? - yes"),
                    collapse = "|")

negate_scope_text <- paste0(c("viewscope: no", "no viewscope", "-Viewscope used? - no", 
                              "scope measurements lost", "no scope"), # scope leaks?
                    collapse = "|")
...
tiering_record_out_path <- "../AquaMatch_harmonize_WQP_old/scratch/data/sdd_analytical_tiering_record.csv"
...
ggsave(filename = "../AquaMatch_harmonize_WQP_old/scratch/data/sdd_charname_dists.png",
       plot = char_dists,
...
grouped_sdd_out_path <- "../AquaMatch_harmonize_WQP_old/scratch/data/sdd_harmonized_grouped.feather"
...
ggsave(filename = "../AquaMatch_harmonize_WQP_old/scratch/data/sdd_tier_dists_postagg.png",
       plot = tier_dists,
...
documented_drops_out_path <- "../AquaMatch_harmonize_WQP_old/scratch/data/sdd_harmonize_dropped_metadata.csv"
...
data_out_path <- "../AquaMatch_harmonize_WQP_old/scratch/data/sdd_harmonized_final.csv"
