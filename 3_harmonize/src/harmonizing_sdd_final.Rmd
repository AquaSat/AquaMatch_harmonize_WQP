...
# update before PR ***
p2_wqp_data_aoi_sdd <- read_feather("../AquaMatch_harmonize_WQP_old/scratch/data/sdd_data.feather")
```
...
dropped_rows <- sdd_fails_removed %>% 
  filter(!grepl("^[0-9.]+$", ResultMeasureValue_original)) %>%
  mutate(ResultMeasureValue_cleaned = str_replace_all(ResultMeasureValue_original, "[0-9. ]", "")) %>%
  group_by(ResultMeasureValue_cleaned) %>%
  summarise(ResultMeasureValue_original = first(ResultMeasureValue_original),
            ResultMeasureValue = first(ResultMeasureValue),
            count = n()) %>% 
  filter(is.na(ResultMeasureValue),
         !is.na(ResultMeasureValue_cleaned)) %>% 
  arrange(desc(count))

write_csv(test1, "../AquaMatch_harmonize_WQP_old/scratch/data/dropped_data.csv")

numeric_dropped_rows <- sdd_fails_removed %>% 
  filter(grepl("[0-9]", ResultMeasureValue_original)) %>% # rows with numeric data
  mutate(ResultMeasureValue_cleaned = str_replace_all(ResultMeasureValue_original, "[0-9. ]", "")) %>% # remove numeric characters
  group_by(ResultMeasureValue_cleaned) %>% # group by the kinds of non-numeric characters that are left over
  summarise(ResultMeasureValue_original = first(ResultMeasureValue_original), # get a count of each group, with example values from original and altered ResulteMeasureValue column
            ResultMeasureValue = first(ResultMeasureValue),
            count = n()) %>% 
  filter(is.na(ResultMeasureValue), # this filters out negative ResultMeasureValue_original values (these are not excluded from the data)
         !is.na(ResultMeasureValue_cleaned)) %>% # this filters out NAs from ResultMeasureValue_original
  arrange(desc(count))

write_csv(numeric_dropped_rows, "../AquaMatch_harmonize_WQP_old/scratch/data/numeric_exclusive_data.csv")

character_dropped_rows <- sdd %>% 
  filter(!grepl("[0-9]", ResultMeasureValue_original)) %>%
...
write_csv(test3, "../AquaMatch_harmonize_WQP_old/scratch/data/character_exclusive_data.csv")

```
- *numbers to keep*
  " (need to convert this)
  ' (need to convert this)
  - (only if its the first item in the)
  <
  >
  ON BOTTOM
  M
  "units in result column match unit column"
...
# ignore result column values that don't have any numeric characters
