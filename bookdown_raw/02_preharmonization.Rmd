...
At the start of the pre-harmonization process the WQP data set contains the parameters `r params_in_use` and `r step_0_rows` rows.
...
Next records that have missing data are dropped from the data set. Several criteria are used when checking for missing data. If any of the below criteria are met the sample is removed from the data set:
...
After dropping `r step_5_dropped` rows the pre-harmonization data set is complete with `r step_5_rows`.
