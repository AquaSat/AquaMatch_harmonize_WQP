...
Following the completion of the download process described in the previous chapter, the pipeline contains raw data for a variety of potential parameters of interest. Before we begin to harmonize each individual parameter we run through a series of "pre-harmonization" steps, which together ensure that each parameter will enter its individual harmonization routine with a dataset that has been formatted identically to each other parameter in the pipeline.
...
At the start of the pre-harmonization process the WQP dataset contains `r step_0_rows` rows.
