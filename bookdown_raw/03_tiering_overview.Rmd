...
## Handling `NA` values
Columns related directly to the interoperability of data in the WQP, specifically field and lab methodology and depth-related columns, often contain many `NA`s. A highly restrictive filtering of the WQP would thus result in very limited data, in part, due to the inconsistent entry of data across data providers. Therefore, we are building this dataset to resolve as many `NA`s as possible, but to also include `NA`s within an "inclusive" data tier when necessary.
...
+ Containing data for one parameter obtained by multiple methods with varying levels of reliability
...
+ **Depth flags**: Flags indicating the completeness of the depth data associated with the measurement
