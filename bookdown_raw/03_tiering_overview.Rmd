...
2. There are cases where the provided date and time zone disagree but `ActivityStartDateTime` uses the provided time zone as-is

We have designed a function, `fill_date_time()`, to create further harmonized date and time columns before the broader data harmonization process. It adds the columns `harmonized_tz`, `harmonized_local_time`, and `harmonized_utc` to the dataset. The function first transforms the provided data into a unified coordinate reference system and then looks up the local time zone for each monitoring site in the dataset using latitude and longitude. The `harmonized_tz` column is then generated using any timezones provided in `ActivityStartTime.TimeZoneCode`, while filling in gaps in timezone data with those identified using the spatial method above. Note that if `ActivityStartTime.TimeZoneCode` listed "UTC" then we also replace this with the spatially-acquired time zone. `harmonized_tz` values are also formatted into a single, location-based Olson timezone format with values like `"America/New_York"`. We then create a text-formatted `harmonized_local_time` column that preserves the local time **and** time zone in a text string (e.g., "2021-09-08 13:27:00 CDT" ). Finally, the local times are converted to UTC and preserved as `harmonized_utc`.
...
A second challenge is that some `ActivityStartTime.TimeZoneCode` values are not in agreement with their `ActivityStartDate`. A hypothetical example would be an `ActivityStartDate` entry of "2023-09-20" with an `ActivityStartTime.TimeZoneCode` value of "CST". This date would fall during daylight saving time ("DST") in the "America/Chicago" time zone and thus one would expect it to be in CDT, not CST. We have learned that recording times in standard time rather than DST is an intentional practice for some field crews and so should not be assumed to be a data entry error. For simplicity's sake, we do not make an attempt to preserve "mismatched" time zones in our harmonization process. This means that our `harmonized_local_time` and `harmonized_utc` values are one hour off of `ActivityStartDateTime` in some instances. If you prefer to use a timestamp that uses the time offset of the original `ActivityStartTime.TimeZoneCode` column (i.e., ignoring DST in some cases), then we suggest using the `ActivityStartDateTime` column, though this will result in having fewer overall rows with complete time data.
