...
Following the completion of the download process described previously, the pipeline contains raw data for a variety of potential parameters of interest. Before we begin to harmonize each individual parameter we run through a series of "pre-harmonization" steps, which ensure that the parameter will enter its individual harmonization routine with a dataset that has been formatted identically to each other parameter in the pipeline.
