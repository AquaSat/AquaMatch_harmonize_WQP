...
The final step in our chla harmonization is flagging field sampling methods based primarily on the `SampleCollectionMethod.MethodName` column. We first classify each record into either *in vitro* or *in situ* methods (i.e., *in vitro* assumes a water sample was collected and taken to a lab for analysis; *in situ* assumes a measurement was obtained in the field). 
...
No records should be removed by this process and so there are `r step_9_dropped` rows dropped leaving `r step_9_rows` remaining in the final harmonized chlorophyll *a* dataset.
