...
# Dissolved organic carbon (DOC) harmonization process
...
After the pre-harmonization process the organic carbon-only WQP dataset contains `r step_0_rows` rows. This dataset contains the following user-defined `characteristicNames`: `r doc_chars`.

The WQP doesn't have a "dissolved organic carbon" `CharacteristicName`, so we use these names in order to capture that parameter.
...
### Filter for water media and doc fraction
...
The first step in DOC harmonization is to ensure that:

1. The media type for the data is `"water"` or `"Water"`
2. The fraction of the carbon pool included is only dissolved organic carbon

To achieve goal number 2 above, we filter the `ResultSampleFractionText` column to include only the following fractions, which correspond with dissolved organic carbon: "Dissolved", "Filterable", "Filtered, lab", "Filtered, field").

Below is a pie chart that documents the number of records that were dropped for each fraction that we omitted:

<br>

<img src="3_harmonize/out/doc_fraction_drop_pie.png" alt="ResultSampleFractionText dropped values" width="650"/>

<br>

Through this process `r step_1_dropped` rows are removed. The final row count after this is `r step_1_rows`.
...
```{r echo=FALSE}
tibble(ResultMeasure.MeasureUnitCode = c('mg/L', 'mg/l', 'ppm', 'ug/l', 'ug/L', 'mg/m3',
