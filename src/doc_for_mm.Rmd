...
raw_doc <- read_feather('data/comb/raw_doc_202210.feather') %>% 
...
aggregated_methods <- raw_doc %>%
  mutate(method_status = case_when(
    ...
    grepl("4500-SiO2 C|4500 SiO2-C|4500 Si-C|4500 Si C|
          ...
          4500-SIO2C",
          analytical_method,ignore.case=T) ~ "Molybdodocte Method",
    ...
    grepl("4500-SiO2 E|4500 SiO2-E|4500 Si-E|4500 Si E|4500E|
          ...
          4500 SIO2 E", 
          analytical_method,ignore.case=T) ~ "Automated Method for Molybdate-Reactive doc",
    
    grepl("4500-SiO2 F|4500 SiO2-F|4500 Si-F|4500 Si F|4500F|
          4500 F|4500-SiF|4500 SIO2 F", 
          analytical_method,ignore.case=T) ~ "Flow Injection Analysis for Molybdate-Reactive docte",
    ...
    # General-ass ICP
    grepl("doc in Water by ICP| ICP, EPA method|ICP Package",
          analytical_method,ignore.case=T) ~ "Just ICP"))

grouped_more <- aggregated_methods %>% 
  mutate(grouped=ifelse(method_status %in% 
                          c('Molybdodocte Method',
                            'Heteropoly Blue Method',
                            'EPA Method 370.1',
                            'EPA Method 366.0',
                            'Automated Method for Molybdate-Reactive doc',
                            'Flow Injection Analysis for Molybdate-Reactive docte')|
...
doc can be analyzed in a myriad of ways; in the water quality portal, doc had `r n_distinct(grouped_more$analytical_method)` unique analytic methods listed for a total of `r nrow(grouped_more)` samples. However, many of these analytic methods can be grouped together into just a few actual methodologies that are realistic for doc:

-   **Colorimetry:** this represents samples that reference the molybdodocte method (SM 4500 C), the heteropoly blue method (SM 4500 D), the automated method for molybdate-reactive doc (SM 4500 E), flow injection analysis for molybdate-reactive doc (SM 4500 F), gas segmented continuous flow colorimetric analysis (EPA 366.0), spectrophotometric detection of dissolved doc (EPA 370.1), "colorimetry", or "molybdate".
...
The ICP method is the second most common method for doc analysis, representing `r grouped_more %>% mutate(total=n()) %>% group_by(grouped) %>% filter(grouped=="ICP") %>% summarize(perc=(n()/total)*100) %>% distinct() %>% ungroup() %>% select(perc) %>% round(digits=3) %>% paste()`% of all doc samples.
...
When looking at these different grouped methodologies coupled with their fractionation we find that most samples were analyzed for the dissolved fraction of doc. In fact, only the heteropoly blue method (a colorimetric technique) had more samples that analyzed the total fraction of doc.
...
There is no clear way of tiering doc based on fraction if we want all tiers to be looking at the exact same thing. Instead, I suggest we only look at samples that are analyzing the dissolved fraction. The dissolved fraction makes up `r grouped_more %>% mutate(total=n()) %>% group_by(aquasat_fraction) %>% filter(aquasat_fraction=="Dissolved") %>% summarize(perc=(n()/total)*100) %>% distinct() %>% ungroup() %>% select(perc) %>% round(digits=3) %>% paste()`% of all doc samples in the Water Quality Portal, a clear sign that the majority of people monitoring doc are looking at the dissolved fraction. With this in mind, I suggest tiering doc as follows:
...
Without any additional steps other than harmonizing units and removing NA/failure-related values (*I also haven't looked at whether values are reasonable*), this is how that data looks:
...
doc_empties_removed <- grouped_more %>%
  ...
  filter(status %in% c('Accepted','Final','Historical','Validated'),
         ...
         # MM THiS IS WHERE WE WANT TO USE YOUR WORD-PULLING CODE!
         !grepl(
           pattern = paste0(
...
#unit_disharmony(vals_cleaned,doc_lookup)
...
# meanscores <- attributes(doc_tiered$grouped)$harmonized_value
# 
# meandf <- data.frame(
...
1.  Is this tiering appropriate? Does not take into account **differences in which method is better for a specific range of values**. **I'm also lumping all ICP methods (ICP-MS, ICP-AES, ICP, etc.) together.**
    -   This approach to tiering tosses out what could be perfectly good data (e.g. high-quality "total" doc). Perhaps not an issue for doc, but this could be a pretty subjective way to tier other metals when we get to them?
