...
harmonize_silica <- function(){
  ...
  raw_silica <- raw_silica %>%
    ...
    filter(type %in% c("Surface Water", "Water", "Estuary", "Ocean Water", 
  ...
  grouped_more <- aggregated_methods %>% 
    mutate(grouped = ifelse(method_status %in% 
                              ...
                              grepl("colorimetr|molybd|colorimeter",
                                    analytical_method,
                                    ignore.case = T),
                            "Colorimetry", ifelse(is.na(method_status) |
    ...
    mutate(grouped = ifelse(method_status %in% c("ICP-AES",
                            ...
                            "ICP",
                            grouped)) %>%
    mutate(aquasat_fraction = case_when(
  ...
  # silica had `r n_distinct(grouped_more$analytical_method)` unique analytic
  # methods listed for a total of `r nrow(grouped_more)` samples. However, many
  ...
  # methods (at `r grouped_more %>% mutate(total=n()) %>% group_by(grouped) %>%
  ...
  # representing `r grouped_more %>% mutate(total=n()) %>% group_by(grouped) %>%
  ...
  # `r grouped_more %>% mutate(total=n()) %>% group_by(grouped) %>%
  ...
  # makes up `r grouped_more %>% mutate(total=n()) %>% group_by(aquasat_fraction) %>% 
