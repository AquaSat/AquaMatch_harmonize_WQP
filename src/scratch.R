...
# duplicates
no_data_samples_1 <- no_duplicate_samples %>%
  # Flag true missing results
  flag_missing_results(., commenttext_missing) %>%
  filter(!flag_missing_result)

nrow(no_duplicate_samples) - nrow(no_data_samples_1)

no_data_samples_2 <- no_data_samples_1 %>%
  filter(is.na(value) &
           is.na(units) &
           is.na(lab_comments) &
           is.na(result_comments)) 

nrow(no_data_samples_1) - nrow(no_data_samples_2)
